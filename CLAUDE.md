...
### 표면 4단계 체계 (22차 §2 — 두 버전 공통 구조)

**컴포넌트는 `surface-0`~`surface-3` 토큰만 참조한다.** 두 브랜치가 같은 이름에 다른 값을
매핑하므로, 색을 바꿔도 레이아웃·기능이 갈리지 않는다.
⚠️ **명도 단계를 다시 좁히지 말 것** — 21차까지의 문제가 "면 구분이 없어 화면이 한 덩어리로
보인다"였고, 그 해법이 이 4단계다. 알파 합성(`bg-xxx/70`)으로 되돌리지도 말 것.

| 토큰 | 역할 | v1 (main) | v3 (design-v3) |
|---|---|---|---|
| `--color-surface-0` | 페이지 배경 | `#041A33` | `#FFFFFF` |
| `--color-surface-1` | 섹션 교대 면 | `#072744` | `#F5F7FA` |
| `--color-surface-2` | 카드·패널 | `#0C3055` | `#FFFFFF`(괘선으로 구분) |
| `--color-surface-3` | 카드 hover·선택 | `#12406E` | `#EAF0F7` |
| `--color-line` | 괘선 | `rgba(214,222,232,.14)` | `#D6DEE8` |
| `--color-line-strong` | 강한 괘선·비율 바 | `rgba(214,222,232,.24)` | `#B9C6D6` |

**섹션 리듬**: 프로필 0 · 관심분야 1 · 제자 진출 0 · 논문 1 · 수상 0.
`data-surface="field"`(=0) / `"paper"`(=1) 훅만 보고 CSS로 처리한다.
섹션 경계에 1px `--line` 가로선 + 그 위 폭 64px 골드 헤어라인(`--hairline`).

**v1 전용 (§2-4)**: 전체 배경 위 2.8% 그레인(`body::before`, SVG feTurbulence) +
8% 비네트(`body::after`). 둘 다 `position:fixed; pointer-events:none`이고
`#root { position:relative; z-index:1 }`로 본문이 그 위에 온다.
그리드 패턴은 **히어로에만** 남긴다.
⚠️ **v3(라이트)에는 그레인·비네트를 넣지 말 것** — `--grain-opacity: 0`, `--vignette: transparent`.

### v1 (main) — 다크 · 대비 실측값

`#041A33`(surface-0) / `#0C3055`(surface-2) 위 기준. 텍스트·포인트·골드는 21차 값 그대로다.

| 색 | 용도 | 판정 |
|---|---|---|
| `#F5F7FA` ink-100 | 본문 | AAA |
| `#D6DEE8` ink-300 | 보조 | AAA |
| `#A8BCD2` ink-500/600 | 약보조 | AA |
| `#DCEAF5` accent-300 | kicker·활성 | AAA |
| `#8FC3F0` accent-400 | 링크·숫자·SSCI | AA |
| `#C6A15B` gold | 골드 텍스트·핀·헤어라인 | AA |
| `#7FC4B4` mint | KCI 라벨 | AA |
| `#5E9BD1` accent-500 | **비텍스트 전용** | 3:1 통과 |

⚠️ **`#5E9BD1`은 바·아이콘·테두리 등 비텍스트에만.**

### v3 (design-v3) — 라이트 · 대비 실측값

| 색 | 용도 | 대비 |
|---|---|---|
| `#263238` ink-100 | 본문 | 13.2:1 |
| `#5A6B78` ink-300/500/600 | 보조·약보조 | 5.5:1 |
| `#003B70` accent-300 | kicker·활성·SSCI | 11.3:1 |
| `#0A4A85` accent-400 | 링크·hover | 9.0:1 |
| `#5E9BD1` accent-500 | **비텍스트 전용** | — |
| `#7A6031` gold | 골드·KCI | 5.9:1 |
| `#F5F7FA` on-band | 파란 띠 위 텍스트 | 13.6:1 (on `#05203E`) |
| `#8FC3F0` | 띠 위 활성 | 7.8:1 (on `#05203E`) |

- **밝은 배경에 `#C6A15B` 사용 금지** (2.4:1) → `#7A6031`.
- **더 밝은 회색(`#8494A0` 등)을 약보조로 쓰지 말 것** — 대비 미달. text-1과 text-2를 같은 값으로 둔다.
- 카드는 면색 대신 **1px `#D6DEE8` 괘선 + `0 1px 2px rgba(38,50,56,.06)`**.
- **본문 중간에 파란 띠를 넣지 말 것.** 파랑은 상단 네비와 하단 푸터(`--band #05203E`)에만.

### SSCI/KCI 토큰

v1 SSCI `#8FC3F0` · KCI `#7FC4B4` / v3 SSCI `#003B70` · KCI `#7A6031`.
**알약 배지를 쓰지 않는다 (22차 §2-5)** — 좌측 3px 바 + 배경 없는 텍스트로만 구분한다.
...
- 서브 필터는 `useSubFilter(papers, primaryKind)` 하나로 통일 —
