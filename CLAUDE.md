...
## 배포 원칙 (29차 §0 — 앞으로 계속 적용)

Vercel은 push할 때마다 커밋 해시가 든 스냅샷 주소를 자동 생성한다. 끌 수 없으므로 **고정 주소 하나만 쓴다.**

- 작업 브랜치는 **`main` 하나뿐이다.** 다른 브랜치에서 작업하거나 push하지 말 것.
- 확인·보고에 쓰는 주소는 **`https://prof-homepage.vercel.app` 하나뿐이다.**
- 커밋 해시가 든 스냅샷 주소(`prof-homepage-xxxxxxx-zerro00.vercel.app`)를 보고에 적지 말 것.
- 작업 후 `git push origin main` → 배포 완료 확인 → **고정 주소를 실제로 열어** 결과를 확인하고 보고한다.
  빌드 통과만으로 판단하지 말 것 — 런타임 오류는 빌드가 잡지 못한다.
...
## 섹션 구성 (순서 고정 — 30차 §2에서 학교 공식 페이지 순서로 변경)
...
**근거**: 학교 공식 페이지의 배열을 따른다 —
`학력·주요경력 → 강의/연구관심분야 → 주요 연구 논문 및 저서 → 주요 학술활동 및 수상 → 기타(졸업생·재학생)`.

- 순서는 `App.jsx`의 `SECTION_IDS` 하나가 **네비 순서와 렌더링 순서를 동시에** 결정한다.
- 섹션 번호(`SectionHeading` 의 `index`)는 손으로 맞춰 둔 값이다 —
  프로필 01 · 관심분야 02 · **논문·저서 03 · 수상·연구비 04 · 제자 진출 05**.
  순서를 바꾸면 이 값도 함께 바꿀 것.
- **해시 문자열은 그대로다** (`#profile #interests #publications #awards #alumni`).
  기존 링크·공유 URL이 깨지지 않아야 하므로 이름을 바꾸지 말 것.
- 히어로 스탯 카드는 섹션을 **id로** 가리키므로(`profile.js`의 `section`) 순서와 무관하다.

**별도의 연락처 섹션이나 대학원생 모집 섹션은 만들지 않는다.** 연락처는 히어로 프로필 안에만.

## 네비게이션 동작

- 메뉴 글자 **17px / weight 500**, 좌우 패딩 20px (모바일 15px / 14px) — 30차 §3.
  4개 언어 모두 1024px에서 줄바꿈·겹침이 없다
  (필요 폭 KO 539 · EN 625 · ZH 657 · JA 691px < 가용 960px).
  그보다 좁아지면 `.nav-scroll`이 가로 스크롤로 처리한다 — 줄바꿈시키지 말 것.
...
## 학교 공식 페이지와의 차이 (30차 §5 — 학교 페이지를 근거로 되돌리지 말 것)

기준: `ysb.yonsei.ac.kr/ysb/faculty/marketing001.do?...articleNo=22036` (2026-06-23 수정).

**원칙 — 이 사이트는 학교 페이지보다 최신을 유지한다.**
새 논문·새 제자 정보가 확인되면 학교 페이지 갱신을 기다리지 말고 반영한다.
단 **반영 근거(DOI·원문·교수님 확인)가 있어야 한다. 추측 반영 금지.**
학교 페이지와 다른 항목은 아래 표에 근거와 함께 남긴다.

| 항목 | 학교 페이지 | 이 사이트 (확정) | 근거 |
|---|---|---|---|
| 재학생 | 김우경·황인서·나규원·김정현·오가령 (5명) | **김우경·황인서·오가령·김연정 (4명)** | 교수님 확인(2026.08.31, 15차). **나규원·김정현을 추가하지 말 것**, 김연정을 빼지 말 것 |
| 논문 편수 | SSCI 36 · KCI 41 (77) | **SSCI 37 · KCI 42 (79)** | 아래 2편 모두 DOI 확인된 실제 게재 논문. 삭제 금지 |

학교 페이지에 없고 이 사이트에만 있는 2편 (**삭제 금지**):
- `Free Versus Paid Over-the-Top Video Streaming Services and the Influence of Social Media` — Journal of Business Research, 2026
- `리셀 시장 내 재활용 소재 활용 제품의 가격 프리미엄 형성: 아식스(Asics) 브랜드를 중심으로` —
  마케팅관리연구 31(2), 75-96, DOI `10.37202/KMMR.2026.31.2.75`

⚠️ **김정현은 2026 아식스 논문의 실제 공저자**라 `authors`에는 이름이 남아 있지만
`studentIds`에는 넣지 않는다(재학생 명단에도 넣지 않는다). 현행 유지.

학교 페이지가 **게재확정**으로 표기 중인 3편 (이 사이트는 연도 표기 유지 — 사실 기록용):
- 인플루언서 커뮤니케이션… (경영정보학연구)
- 상권 내 유통 채널 경쟁… (프랜차이징저널)
- 프랜차이즈 가맹 본부의 위기… (프랜차이징저널)
...
- **화면에서는 교수님 공저 논문과 한 목록으로 합쳐 보여준다 (29차 §2 — 아래 「제자 실적 라벨」 참조).**
  `priorWorks`가 없는 제자에게는 그만큼 항목이 줄 뿐, 라벨은 모든 제자가 동일하다.
...
### 제자 실적 라벨 — 모든 제자가 같은 문구 (29차 §2)

- 라벨은 **`연구 실적 N편 보기` / `연구 실적 N편 접기` 하나뿐이다.**
  `공저`·`합류 전`·`이전 연구` 같은 구분 문구를 **화면에 두지 말 것.**
- `worksForStudent(personId)`(교수님 공저)와 `entry.priorWorks`를 **화면에서만** 합쳐
  하나의 목록으로 보여준다. 정렬은 **연도 내림차순**, 총 편수를 라벨에 쓴다.
  컴포넌트는 `WorksAccordion({ works, priorWorks })` 하나다 (`PriorWorksAccordion`은 폐지).
- 4개 국어: `연구 실적` / `Publications` / `研究成果` / `研究業績`
  (키는 `students.worksShow/worksHide`의 `_one`·`_other`. `priorWorks*` 키는 삭제했다).
- ⚠️ **집계 규칙은 바뀌지 않았다.** 화면 라벨만 통일한 것이다:
  교수님 논문 총 편수 **79 유지** · 실적 배지(`SSCI n · KCI n`)는 **교수님 공저만** 집계 ·
  `priorWorks`는 논문 섹션의 목록·검색·필터에 나타나지 않음 ·
  `check-authorship.mjs`의 김연정 EXPECTED는 **SSCI 0 · KCI 0** 그대로.
...
**작업은 `main`에서만 한다 (29차 §0).** 나머지는 전부 보관용이다.

| 브랜치 | 내용 | 상태 |
|---|---|---|
| `main` | **최종 확정본.** 29차에서 design-v3를 fast-forward로 승격했다 | 운영 — 유일한 작업 브랜치 |
| `archive/main-old` | 29차 승격 직전의 main (`23008ff`) 백업 | 보관 (삭제 금지) |
| `design-dark` | 24차 직전의 딥 네이비 다크 버전 백업 | 비교용 보관 |
| `design-v3` | 29차에 main으로 승격됐다. main과 내용이 동일하다 | 역할 종료 |

- `design-v2`는 폐기했다. 되살리지 말 것.
- **`archive/main-old`·`design-v3`를 임의로 삭제하지 말 것.** 정리 방안은 사용자 확인 후 진행한다.
- `design-dark`에는 main에 없는 커밋 2건(다크 팔레트 복구)이 남아 있다.
  **main으로 머지하지 말 것** — 확정된 밝은 배경을 되돌리게 된다.
- **새 Vercel 프로젝트를 만들지 말 것** (Preview 접근 문제는 Vercel 설정 쪽 사안이다).
...
29차에서 `design-v3`의 색·기능을 **`main`으로 승격**했다. 지금 `main`에 있는 것이 최종안이다.
`src/index.css`가 색의 유일한 출처이며, 아래는 그 파일이 실제로 담고 있는 확정 값이다.

- **본문 배경은 흰색.** 파랑은 **상단 네비와 하단 푸터에만** (`--band: #05203E`,
  글씨 `--on-band: #F5F7FA`, 활성 `--on-band-active: #8FC3F0`).
  본문 중간에 파란 띠를 만들지 말 것.
- 표면 4단계: `surface-0 #FFFFFF` / `surface-1 #F5F7FA` / `surface-2 #FFFFFF` / `surface-3 #EAF0F7`.
  카드는 면색이 아니라 **1px 괘선**으로 구분한다.
- **밝은 배경에 `#C6A15B` 골드 금지** (2.4:1) → `#7A6031`.
- **그레인·비네트 금지** — `--grain-opacity: 0`, `--vignette: transparent`. 발광·글로우도 전부 꺼져 있다.
- 히어로 사진 테두리는 골드 대신 1px 괘선(`[class~="border-gold-400/30"]` 오버라이드).
- 대비 실측값 표는 아래 「팔레트」 절에 있다. **재계산하지 말 것.**
...
- **네비·푸터 (27차 §0 — 교수님 확정, 바꾸지 말 것)**: 딥네이비 띠 `--band #05203E`,
  글씨 `--on-band #F5F7FA`, 활성 탭·포인트 `--on-band-active #8FC3F0`.
  **본문은 흰 배경 그대로다** — 파란 면은 상단 네비와 하단 푸터에만 두고,
  본문 중간에 파란 띠를 만들지 말 것.
...
### 서브 필터 드롭다운 (27차 §2 — 커스텀 컴포넌트, 되돌리지 말 것)

`Dropdown`(`src/components/Publications.jsx`). **네이티브 `<select>` 금지** — 펼치면 OS 기본
메뉴가 떠서 사이트와 따로 논다. `<button>` + `role="listbox"` 커스텀 패널로 만든다.

- **닫힌 상태**: 높이 40px, 배경 `#FFFFFF`, 1px `#E2E6EB`, 라운드 8px.
  라벨 14px `#1A1A1A` · 개수 `#5A6472` · 우측 chevron 16px `#5A6472`.
  hover 테두리 `#C8CFD8`, 포커스 테두리 `#003876` + 2px 아웃라인.
  **좌측 `FILTER` 영문 라벨은 두지 않는다.**
- **펼친 패널**: 배경 `#FFFFFF`, 1px `#E2E6EB`, 라운드 10px,
  그림자 `0 4px 16px rgba(26,26,26,.10)`, 상단 6px 여백, 최대 높이 320px(내부 스크롤).
  항목 사이 구분선 없음. 열림/닫힘 120ms 페이드 + 4px 슬라이드 — 그 이상 금지.
- **항목**: 높이 38px(모바일 44px), 좌우 패딩 14px, 14px. `이름 (개수)`.
  hover 배경 `#F0F4F9`.
  **선택 항목은 배경 `#E8EEF5` + 좌측 3px `#003876` 바 + 텍스트 `#003876`** —
  체크 아이콘을 쓰지 않는다. 좌측 저널 목록의 선택 행과 같은 언어여야 한다.
- 첫 항목은 `전체 (N)`, 이후 편수 내림차순(연도는 최신순).
  **연도는 구간**으로 묶는다: `2026– / 2021–2025 / 2016–2020 / 2010–2015`.
- 키보드: `Enter`/`Space` 열기, `↑↓` 이동, `Enter` 선택, `Esc` 닫기.
  `aria-expanded`·`aria-selected` 필수. 바깥 클릭 시 닫힘.
- **세 뷰(키워드·저널·방법론)가 같은 컴포넌트를 쓴다.** 뷰별로 따로 만들지 말 것.

### `Top Journal` 배지 (27차 §4)

- 대상은 **`JOURNAL_RANK`가 1인 저널만** — JMR · Marketing Science · JCR ·
  Management Science · IJRM · JIBS. **rank 2·3 저널에 붙이지 말 것.**
- 위치는 논문 카드 상단 `SSCI` 라벨 바로 뒤.
- 스타일: 배경 `#F5F1E8`, 텍스트 `#7A6031`, 1px `#E5DCC8`, 라운드 4px, 12px,
  좌우 패딩 8px. ★ 12px 같은 색.
- **좌측 저널 목록에는 배지를 넣지 않는다** — 위상은 정렬 순서로 이미 표현된다.
- i18n 키 `pubsUI.topJournalBadge`: `Top Journal` / `Top Journal` / `顶级期刊` / `トップジャーナル`.

### 저자 줄 표시 변환 (27차 §5)

한 논문 안에서 `황인서, Li Yiling, 최정혜`처럼 한글·영문이 섞여 나오던 것을 화면에서만 통일한다.

- **`publications.js`의 `authors` 배열은 검증 근거다. 원문 표기 그대로 두고 절대 수정하지 않는다.**
  (`check-authorship.mjs` 규칙 5가 이 원문 표기로 배정을 검증한다)
- **표시할 때만** 치환한다 — `studentIds`에 등록된 인물은 현재 언어의 표준 이름으로:
  한국어 `황인서, 이예령, 최정혜` / 영문 `Inseo Hwang, Yiling Li, Jeonghye Choi`.
- 등록되지 않은 외부 공동연구자는 원문 표기 그대로 둔다.
- 제자 이름만 링크(제자 진출로 이동), 나머지는 일반 텍스트 — 판정은 `authorsWithLinks(paper)`.
- 변환 대응표는 위 「한국어 ↔ 영문 이름 매핑」 표를 쓴다. **새 이름을 만들어내지 말 것.**

### 방법론 미표기 정책 (27차 §8)

- 방법론이 빈 배열인 논문을 **감추지 않는다.** 좌측 목록 맨 아래에 그대로 노출한다.
- 문구는 `방법론 미표기 N편` (`분류 예정`이 아니다).
- 바로 아래 한 줄 설명을 4개 언어로 둔다:
  `논문 원문에서 방법론이 명시된 경우에만 분류했습니다.`
- **추측으로 방법론을 채우지 말 것.** 근거(`methodSource`)가 없으면 빈 배열이 정답이다.
...
- **full-bleed를 쓰지 않는다 (29차 §3).** 지도는 **본문 컨테이너 폭** 안에 둔다.
  `mx-[calc(50%-50vw)]`로 되돌리지 말 것 — 지도가 화면을 압도한다.
  25차에 좌측 명단 패널을 없앴으므로 지도가 섹션 폭을 그대로 쓴다.
  지도 영역 최소 높이 `xl` **420px** / 1440px 이상 **480px**, 모바일 **300px**.
- **도시 라벨 13px**(weight 500, 1px 외곽), 색 `--map-label`. **더 키우지 말 것** —
  지도가 작아져 상대적으로 이미 커 보인다. 핀 반지름 **5px** /
  교수 임용 **6px** + 외곽 링 8.5px / hover 7px. 겹침 판정 **8px** · 분리 **6px**.
  ⚠️ 투영(`scale 240` / `center [15,12]`)과 viewBox는 그대로라 라벨의 **상대 배치는 변하지 않는다.**
  `alumni.js`의 `labelDx/labelDy`는 손으로 잡은 값이다 —
  폰트 크기나 투영을 바꾸면 렌더 후 경계상자로 겹침을 다시 확인할 것.
- **호(arc)**: 서울 → 해외 8개 도시. 가까운 곳부터 120ms 간격, 500ms씩 그려 총 1.34초.
  선 **0.8px** / 기본 **22%**(교수 임용 28%), hover 시 해당 선만 60% 나머지 8%.
...
- ~~김연정 영문 표기 미확인~~ → **28차에서 `Yeonjeong Kim`으로 확정** (DBpia 저자 ID 385227137).
  (곽유신도 JBR 2026 OTT 논문의 Crossref 메타데이터에서 `Youshin Kwak`으로 확정 — 둘 다 해소)
