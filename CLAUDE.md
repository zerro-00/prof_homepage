...
| 브랜치 | 내용 | 상태 |
|---|---|---|
| `main` | **밝은 배경 (24차 §4)** — 학교 페이지 계열. 이것이 기준 버전이다 | 운영 |
| `design-dark` | 24차 직전의 딥 네이비 다크 버전 백업 | 비교용 보관 |
| `design-v3` | 22차까지의 라이트 실험 브랜치. main이 밝아지면서 역할이 끝났다 | 정리 대상 |

- `design-v2`는 폐기했다. 되살리지 말 것.
- **`design-v3`는 임의로 삭제하지 말 것.** 정리 방안은 사용자 확인 후 진행한다.
...
## ⚠️ 이 브랜치는 `design-v3` — 밝은 배경 (22차 §2-2)

`main`에서 분기했다. **레이아웃·기능은 main과 100% 동일하고 색만 다르다.**
`git diff main design-v3 --stat` = `src/index.css` + `CLAUDE.md` 두 파일이어야 한다.

- **컴포넌트를 건드리지 말 것.** 기능·구조 변경은 main에서 하고 v3로 머지한다.
  머지 때 `src/index.css`는 `git checkout main -- src/index.css`로 되돌린 뒤
  이 브랜치의 §2-2 팔레트를 다시 얹는다.
- main과 **같은 토큰 이름(`surface-0`~`surface-3` 등)에 밝은 값만 매핑한다.**
  새 토큰 이름을 만들지 말 것 — 컴포넌트가 main과 같은 클래스를 쓰기 때문이다.
- **본문 배경은 흰색.** 파랑은 상단 네비와 하단 푸터(`--band #05203E`)에만.
  본문 중간에 파란 띠를 넣지 말 것.
- **밝은 배경에 `#C6A15B` 골드 금지** (2.4:1) → `#7A6031`.
- **그레인·비네트 금지** — `--grain-opacity: 0`, `--vignette: transparent`.
  발광·글로우도 전부 껐고, 카드는 1px `#D6DEE8` 괘선 + `0 1px 2px rgba(38,50,56,.06)`.
- 히어로 사진 테두리는 골드 대신 1px 괘선(`[class~="border-gold-400/30"]` 오버라이드).
- 대비 실측값 표는 아래 「v3 (design-v3) — 라이트」 절에 있다.
