...
| | 브랜치 | 링크 | 내용 |
|---|---|---|---|
| **v1** | `main` | https://prof-homepage.vercel.app (기존 링크 유지) | 기준 버전 |
| **v3** | `design-v3` | Vercel Preview | **v1과 레이아웃·기능 100% 동일, 색만 다름** |
...
형광 파랑을 걷어내고 채도를 낮춘 슬레이트 블루로 교체했다. **포인트 컬러를 다시 형광 계열로 올리지 말 것.**

| 토큰 | 값 | 카드 배경(#0E1729) 대비 |
|---|---|---|
| `--color-ink-100` | `#EEF1F7` | 15.82:1 |
| `--color-ink-300` | `#AFBACD` | 9.14:1 |
| `--color-ink-500` | `#7E8AA0` | 5.14:1 |
| `--color-ink-600` | `#7A86A3` | 4.91:1 |
| `--color-accent-300` | `#A9C0E0` | 9.63:1 |
| `--color-accent-400` | `#8FADD6` | 7.77:1 (링크·활성 언더라인·kicker) |
| `--color-accent-500` | `#5B7CAE` | 4.21:1 (바·아이콘 등 비텍스트) |
| `--color-gold-300/400` | `#DCC28C` / `#C9A96E` | 10.35 / 8.00 |
| `--color-mint-400` | `#7FB3A8` | 7.60:1 |

지령서의 `--color-ink-600 #626D82`는 실측 3.43:1로 본문 기준에 못 미쳐 `#7A86A3`(4.91:1)로 조정했다.
**골드는 교수 임용 핀 · 수상 ★ 전용.**
...
| 토큰 | 값 | 대비 |
|---|---|---|
| `--ssci-text` | `#8FADD6` | 7.77:1 |
| `--ssci-bar` | `#5B7CAE` | 4.21:1 |
| `--kci-text` | `#7FB3A8` | 7.60:1 |
| `--kci-bar` | `#5F9A8E` | 5.54:1 |
...
- **호(arc)**: 서울 → 해외 8개 도시. 가까운 곳부터 120ms 간격, 500ms씩 그려 총 1.34초.
  선 0.8px / 기본 22%(교수 임용 30%), hover 시 해당 선만 60% 나머지 8%.
...
- 8px 이내로 겹치는 핀은 6px 벌리고 1px 선으로 실제 좌표와 연결한다.
