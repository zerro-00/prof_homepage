...
| 브랜치 | 목적 |
|---|---|
| `main` | 운영 기준. 16차 §1 공통 수정만 반영, **UI 리뉴얼 없음** |
| `design-v2` | 디자인 리뉴얼 — **색은 main의 현재 팔레트 그대로** |
| `design-v3` | design-v2 + 연세 팔레트 색 교체. **레이아웃·컴포넌트는 v2와 100% 동일** |

- **`design-v2`/`design-v3`를 `main`에 머지하지 말 것.** 사용자가 세 배포를 비교해 고른다.
- 공통 수정은 `main` → `design-v2` → `design-v3` 순으로 전부 반영한다.
- v2에서 색을 바꾸지 말 것. v3에서 레이아웃을 바꾸지 말 것.
